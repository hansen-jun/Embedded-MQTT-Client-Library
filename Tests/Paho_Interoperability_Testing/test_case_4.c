...
static uint8_t const*   PublishTopic    =   (uint8_t*)"test/qos_1";
static uint8_t const*   PublishMessage  =   (uint8_t*)"This is a test message for qos1.";
...
extern int32_t TestCase_004(S_USER_DATA* Ctx)
{
    ...
    do
    {
        ...
        Err = MQC_Publish(Ctx->Handler, &Message, E_MQC_QOS_1, false, PublishResponseFunc);
        if( D_MQC_RET_OK != Err)
        {
